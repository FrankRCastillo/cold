...
class Interface:
    def __init__(self, config, query):
        ...
        self.page_param_name = self.page_params['name']
        self.page_param_base = self.get_param_base(self.page_param_name)
        ...
        exit_key = getattr(curses, "KEY_EXIT", None)
        if exit_key is not None:
            self.key_cmds[exit_key] = self.quit
        ...
        while not self.end_prog:
            try:
                ...
                if self.user_input in self.results.keys():
                    ...
                    self.win_page       = 1
                    self.last_page      = 1
                    self.params[self.page_param_name] = self.page_param_base
    ...
    def load_results(self):
        ...
        page_span = self.parser.get_page_span(self.max_rows)
        win_gt_last_page = self.win_page + 1 > self.last_page
        rslt_len_ge_span = self.results_len >= page_span * self.win_page
        self.last_page   = self.win_page + 1 if win_gt_last_page and rslt_len_ge_span else self.last_page
    ...
    def poll_escape(self):
        ...
        if key in (27, getattr(curses, "KEY_EXIT", -1)):
            self.quit()
            return True
    ...
    def get_param_base(self, param_name):
        param_val = self.params.get(param_name)

        if isinstance(param_val, list):
            return param_val[0] if param_val else 0

        return 0 if param_val is None else param_val
